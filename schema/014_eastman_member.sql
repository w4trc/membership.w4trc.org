-- Migration 014: Eastman member tracking
-- Tracks whether a member is an Eastman Chemical employee.
-- Eastman pays the club $3/yr per Eastman member; Eastman members owe $0 in dues.
-- Rebuilds memberships to add 'eastman' as a valid payment_method.

ALTER TABLE members ADD COLUMN is_eastman_member INTEGER NOT NULL DEFAULT 0;

CREATE INDEX IF NOT EXISTS idx_members_eastman ON members(is_eastman_member);
...
CREATE TABLE memberships_new (
  ...
  amount_paid          REAL,
  paid_date            TEXT,
  payment_method       TEXT    CHECK (payment_method IN ('cash', 'check', 'paypal', 'stripe', 'eastman', 'other', NULL)),
  check_number         TEXT,
  notes                TEXT,
